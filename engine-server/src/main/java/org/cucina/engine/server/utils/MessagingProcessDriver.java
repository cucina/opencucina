...
public class MessagingProcessDriver extends MessagingGatewaySupport implements ProcessDriver {
	...
	public MessagingProcessDriver(
			...
			ContextService contextService, RegistryUrlExtractor registryUrlExtractor) {
	...
	@Autowired
	@Override
	public void setRequestChannel(@Qualifier("conversation") MessageChannel conversation) {
		super.setRequestChannel(conversation);
	}

	@Autowired
	@Override
	public void setReplyChannel(@Qualifier("conversationReply") MessageChannel conversationReply) {
		...
		for (Operation action : actions) {
			...
			if (action instanceof OperationDescriptor) {
				...
				if (result instanceof Map<?, ?>) {
					executionContext.getParameters().putAll(
	...
	private Object process(AbstractElementDescriptor descriptor, ExecutionContext context) {
		...
		descriptor.put(
				"domainId",
				(po instanceof EntityDescriptor) ? ((EntityDescriptor) po).getRemoteId() : po
						.getId());
	...
	private Object sendAndReceive(ProcessElementDto wed, ExecutionContext ec) {
		...
		Message<CallbackEvent> message = MessageBuilder
				.withPayload(event)
				.setHeader(Operative.CONVERSATION_ID, contextService.get(Operative.CONVERSATION_ID))
				.setHeader(Operative.DESTINATION_NAME, registryUrlExtractor.findDestinationName(wed.getApplication()))
				.build();
