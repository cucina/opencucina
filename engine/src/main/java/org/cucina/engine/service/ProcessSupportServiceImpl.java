 ...
 */
@Service
public class ProcessSupportServiceImpl implements ProcessSupportService {
	private static final Logger LOG = LoggerFactory
			.getLogger(ProcessSupportServiceImpl.class);
	 ...
	 * JAVADOC Method Level Comments
	 *
	 * @param historyRecordRepository
	 *            JAVADOC.
	 */
	@Required
	@Autowired
	public void setHistoryRecordRepository(
	...
	public Collection<Transition> listActionableTransitions(String workflowId) {
		ProcessDefinition definition = definitionService
				.loadDefinition(workflowId);

		if (definition == null) {
			if (LOG.isInfoEnabled()) {
				LOG.info("No workflow found for workflowId [" + workflowId
						+ "], returning");
			}
			...
			if (place instanceof Station) {
				...
				if (!StringUtils.isEmpty(systemPrivilege)) {
					for (Transition transition : transitions) {
						...
						if (transition.getPrivilegeNames().contains(
								systemPrivilege)) {
	...
	@Override
	@Transactional
	public Map<Long, Collection<String>> listAllTransitions(
			Collection<Long> ids, String applicationType) {
		Collection<ProcessToken> tokens = tokenRepository
				.findByApplicationTypeAndIds(applicationType,
		...
		if (CollectionUtils.isNotEmpty(tokens)) {
			for (ProcessToken token : tokens) {
				results.put(token.getDomainObjectId(), processEnvironment
						.getService().listTransitions(token, null));
			}
		}
	...
	@Override
	@Transactional
	public Collection<String> listTransitions(Collection<Long> ids,
	...
	@Override
	@Transactional
	public Collection<Map<String, Object>> listWorkflowProperties(
			Collection<Long> ids, String applicationType) {
		...
		SearchBean searchBean = searchBeanFactory.buildSearchBean(
				applicationType, params);
		...
		List<Map<String, Object>> results = searchResults.searchMap(0,
				Integer.MAX_VALUE);

		if (LOG.isDebugEnabled()) {
			LOG.debug("Returning [" + results.size() + "] for ids ["
					+ ids.size() + "]");
		}
	...
	public Collection<Map<String, String>> loadTransitionInfo(String processId) {
		if (LOG.isDebugEnabled()) {
			LOG.debug("Loading process states by transitionId for process ["
					+ processId + "]");
		}

		ProcessDefinition definition = definitionService
				.loadDefinition(processId);

		Assert.notNull(definition, "definition should be found with id ["
				+ processId + "]");
			...
			for (int i = 0; i < places.length; i++) {
				...
				if (CollectionUtils.isNotEmpty(place.getAllTransitions())) {
					for (Transition transition : place.getAllTransitions()) {
						...
						stateTransitionMap
								.put("transition", transition.getId());
						stateTransitionMap.put("stateFrom", transition
								.getInput().getId());
						stateTransitionMap.put("stateTo", transition
								.getOutput().getId());
	...
	public void makeTransition(Map<Long, Integer> entities,
		...
		try {
			Collection<ProcessToken> tokens = tokenRepository
					.findByApplicationTypeAndIds(applicationType, entities
							.keySet().toArray(new Long[entities.size()]));
			...
			for (ProcessToken token : tokens) {
				...
				token = (ProcessToken) processEnvironment.getService()
						.executeTransition(token, transitionId, parameters);

				if (processEnvironment.getProcessDefinitionHelper().isEnded(
						token)) {
	...
	public void makeTransition(Long id, String applicationType,
			String transitionId, String comment, String approvedAs,
			String assignedTo, Map<String, Object> extraParams,
			Attachment attachment) {
		try {
			Collection<ProcessToken> tokens = tokenRepository
					.findByApplicationTypeAndIds(applicationType, id);
	...
	public List<HistoryRecord> obtainHistory(Long id, String applicationType) {
		return tokenRepository
				.findHistoryRecordsByDomainObjectIdAndDomainObjectType(id,
						applicationType);
	}
	...
	public List<Map<Object, Object>> obtainHistorySummary(Long id,
			String applicationType) {
		...
		List<HistoryRecord> hres = historyRecordRepository
				.findByIdAndApplicationType(id, applicationType);
	 ...
	 */
	@Override
	public Token startWorkflow(PersistableEntity entity,
	...
	public Token startWorkflow(PersistableEntity entity, String processId,
		...
		ProcessToken token = (ProcessToken) processEnvironment.getService()
				.startProcess(entity, processId, null, parameters);
	...
	private Token loadLastToken(Collection<Long> ids, String applicationType) {
		...
		for (Token tokenItem : tokens) {
			Assert.notNull(tokenItem.getPlaceId(),
					"Token does not have a placeId");
